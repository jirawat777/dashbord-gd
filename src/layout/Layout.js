...
export default function Layout() {
  ...
  return (
    <>
      ...
      <Box
        className={`${classes.color_header} flex justify-between items-center sm:hidden `}
      >
      ...
      >
        <div className="grid grid-rows-2 grid-flow-col sm:grid-rows-3 sx:grid-flow-col  gap-2 flex">
          {route.map((r, i) => (
            <Box
              key={i}
              className={`${classes.block_menu} w-[120px] h-[120px] rounded-lg hover:shadow-inner`}
              onClick={() => handleClickPath(r)}
            >
